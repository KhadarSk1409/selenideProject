...
public class AreaHeaderTest extends BaseTest {
    ...
    @Test
    @DisplayName("Getting to Form Dashboard - OPTION 01")
    @Order(1)
    public void gettingToFormDashboardOption1() {
        ...
        $("#root div.MuiCollapse-container.MuiCollapse-entered nav  > div > div").should(exist);
        List<SelenideElement> listFavForms = $$("#root div.MuiCollapse-container.MuiCollapse-entered nav  > div > div");
        int listFavFormsCount = listFavForms.size();
        assertTrue((listFavFormsCount > 1), "Verify that Favorite forms are available"); //Verify that Favorite form is available

        for (int i = 0; i < listFavFormsCount; i++) {
            String formName = listFavForms.get(i).getText();
            if (formName.contains("NR Test Form")) //The array element should not be NR Test form
                continue;
            else
                listFavForms.get(i).click(); //Click on the Favorite form
    ...
    @Test
    @DisplayName("Getting to Form Dashboard - OPTION 02")
    @Order(2)
    public void gettingToDashboardOption2() {
...
//       $("#cUsageOverview").$(byAttribute("title","Copy URL to Clipboard")).click(); //Copy url ??
        // String copyUrl = KeyEvent ??
    ...
    public void verifyButtonNotifications() {
        ...
        $("notificationsCard").should(exist).click(); //Notifications card and click on it
        $("notificationsCard").should(exist); //Notifications card still exists
